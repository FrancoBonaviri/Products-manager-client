...
import { confirmar, getAll as getAllVentas, getDetallesByVentaCode } from '../../../services/ventaService';
...
export const ListSolicitudesVentas = () => {
                                ...
                                <div className="table-responsive" >
                                    <table className="table color-table primary-table">
                                        ...
                                        <tbody >
                                            {
                                                ventas.map( (venta, i) => (
                                                    <ListSolicitudVentaItem key={ i } venta={ venta } setUpdaterObject={ setUpaterObject } />
                                                ))
                                            }
                                        </tbody>
                                    </table>
...
const ListSolicitudVentaItem = ({venta, setUpdaterObject}) => {

    const [total, setTotal] = useState(0)
    const dispatch = useDispatch();

    const handleConfirmClick = () => {
        dispatch( openModal( <ConfirmVentaModal  onConfirm={ onConfirm } onCancel={ () => dispatch( closeModal() )} />, 'Confirmar venta' ) ) 
    }
    
    const onConfirm = () => {
        confirmar( venta.codigo )
        .then( res => {
            dispatch( openAlert('success','La venta fue confirmada con exito, podes encontrarla en el listado de ventas pendientes'));
    ...
    return (
        <tr>
            ...
            <td><span style={{ cursor: 'pointer' }} onClick={ handleConfirmClick } className="badge bg-info">Confirmar venta</span></td>
        </tr>
    )
...
const ConfirmVentaModal = ({ onConfirm, onCancel, ventaCode }) => {
    ...
    return (
        <>
            <div className="modal-body">    
                <div className='container'>
                    <p> Esta a punto de confirmar la venta { ventaCode }.</p>
